#include "pch.h"
#include "DataReaderFunctions.h"
#include "XsvReader.h"

bool TjsWeather::coordinateIsIn5kmBlock(int const coordinate, std::string const& blockCentreCoordinateAsString)
{
...
TjsWeather::DaySeries TjsWeather::loadFromMetOfficeGriddedData(std::istream& stream,
   int const easting, int const northing)
{
   ...
   while (stream)
   {
      ...
      {
         result.emplace_back(dataLine[0], dataLine[localColumnIndex]);
      }
